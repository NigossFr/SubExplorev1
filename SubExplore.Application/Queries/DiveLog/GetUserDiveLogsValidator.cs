...
/// <summary>
/// Validator for GetUserDiveLogsQuery.
/// </summary>
public class GetUserDiveLogsValidator : AbstractValidator<GetUserDiveLogsQuery>
{
    private static readonly string[] ValidSortFields = { "DiveDate", "MaxDepth", "Duration" };

    /// <summary>
    /// Initializes a new instance of the <see cref="GetUserDiveLogsValidator"/> class.
    /// </summary>
    public GetUserDiveLogsValidator()
    {
        RuleFor(x => x.UserId)
        ...
        RuleFor(x => x.DiveType)
            .InclusiveBetween(0, 7).WithMessage("Dive type must be between 0 and 7")
            .When(x => x.DiveType.HasValue);

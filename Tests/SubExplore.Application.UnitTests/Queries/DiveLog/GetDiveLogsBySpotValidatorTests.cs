...
public class GetDiveLogsBySpotValidatorTests
{
    private readonly GetDiveLogsBySpotValidator _validator = new();
    ...
    [Fact]
    public void Validate_EmptyDivingSpotId_ShouldHaveValidationError()
    {
        var query = new GetDiveLogsBySpotQuery(Guid.Empty);

        var result = _validator.TestValidate(query);

        result.ShouldHaveValidationErrorFor(x => x.DivingSpotId)
            .WithErrorMessage("Diving spot ID is required");
    }
